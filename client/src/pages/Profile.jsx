    ...
    <div className="page">
          ...
          <div>
            <div style={{ padding: "30px", border: "1px solid rgba(12,15,29,0.1)", borderRadius: "10px", backgroundColor: "#fff", marginBottom: "20px" }}>
              <div style={{ display: "flex", alignItems: "center", marginBottom: "30px" }}>
                <div style={{ width: "100px", height: "100px", borderRadius: "50%", overflow: "hidden", backgroundColor: "var(--accent)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "3rem", marginRight: "20px", flexShrink: 0 }}>
                  {user.imageURL
                ...
                <div>
                  ...
                  <p style={{ margin: "8px 0 0", fontSize: "0.85rem", color: "var(--muted)" }}>Role: {user.role}</p>
                </div>
              </div>

              {user.contactNumber && (
                <div style={{ marginBottom: "20px" }}>
                  <p style={{ margin: "0 0 8px", color: "var(--muted)", fontSize: "0.9rem" }}>Phone</p>
              ...
              {user.dateOfBirth && (
                <div style={{ marginBottom: "20px" }}>
                  <p style={{ margin: "0 0 8px", color: "var(--muted)", fontSize: "0.9rem" }}>Date of Birth</p>
            ...
            <div style={{ padding: "20px", border: "1px solid rgba(12,15,29,0.1)", borderRadius: "10px", backgroundColor: "#fff" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
                <div>
                  ...
                  <div style={{ display: "inline-block", padding: "6px 12px", backgroundColor: user.status === "active" ? "#4caf50" : "#ff9800", color: "#fff", borderRadius: "20px", fontSize: "0.85rem", fontWeight: "600" }}>
                    {user.status === "active" ? "Active" : "Inactive"}
                  </div>
                </div>
                <div>
                  <p style={{ margin: "0 0 8px", color: "var(--muted)", fontSize: "0.85rem" }}>Joined</p>
          ...
          <div className="grid">
            <div className="form-group">
              ...
              <input id="firstName" type="text" name="firstName" placeholder="First name"
                ...
                className={profileErrors.firstName ? "error" : ""} disabled={profileLoading} />
            ...
            <div className="form-group">
              ...
              <input id="lastName" type="text" name="lastName" placeholder="Last name"
                ...
                className={profileErrors.lastName ? "error" : ""} disabled={profileLoading} />
          ...
          <div className="grid">
            <div className="form-group">
              ...
              <input id="contactNumber" type="tel" name="contactNumber" placeholder="+1 (555) 123-4567"
                value={formData.contactNumber} onChange={handleProfileChange} disabled={profileLoading} />
            </div>
            <div className="form-group">
              <label htmlFor="dateOfBirth">Date of Birth</label>
              <input id="dateOfBirth" type="date" name="dateOfBirth"
                value={formData.dateOfBirth} onChange={handleProfileChange} disabled={profileLoading} />
            </div>
          </div>
              ...
              {(imagePreview || user?.imageURL) && (
                <div style={{ position: "relative" }}>
                  <img
                    ...
                    alt="Profile preview"
                    style={{ width: "60px", height: "60px", borderRadius: "50%", objectFit: "cover", border: `2px solid ${imagePreview ? "#ff9800" : "var(--accent)"}` }}
                  />
                  {imagePreview && (
                    <span style={{ position: "absolute", top: "-6px", right: "-6px", background: "#ff9800", color: "#fff", borderRadius: "50%", width: "18px", height: "18px", fontSize: "0.65rem", display: "flex", alignItems: "center", justifyContent: "center" }} title="Pending — not saved yet">!</span>
                  )}
                </div>
              )}
              <label className="btn ghost" style={{ cursor: "pointer", padding: "8px 16px", fontSize: "0.9rem" }}>
                {pendingImage ? `✎ ${pendingImage.name.slice(0, 20)}…` : (user?.imageURL ? "Change Photo" : "Choose Photo")}
                <input type="file" accept="image/*" onChange={handleImageSelect} disabled={profileLoading} style={{ display: "none" }} />
              </label>
              {pendingImage && <span style={{ fontSize: "0.8rem", color: "#ff9800" }}>⚠ Not saved yet — click Save Changes</span>}
              {!pendingImage && user?.imageURL && <span style={{ fontSize: "0.85rem", color: "var(--muted)" }}>Photo saved ✓</span>}
            </div>
          </div>
          ...
          <div className="form-group">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "15px", flexWrap: "wrap" }}>
              {user?.resumeURL && !pendingResume && (
                <a href={resolveUrl(user.resumeURL)} target="_blank" rel="noopener noreferrer" className="btn ghost" style={{ padding: "8px 16px", fontSize: "0.85rem" }}>
                  📄 View current resume
                </a>
              )}
              <label className="btn ghost" style={{ cursor: "pointer", padding: "8px 16px", fontSize: "0.9rem" }}>
                {pendingResume ? `✎ ${pendingResume.name.slice(0, 24)}…` : (user?.resumeURL ? "Replace Resume" : "Choose Resume")}
                <input type="file" accept=".pdf" onChange={handleResumeSelect} disabled={profileLoading} style={{ display: "none" }} />
              </label>
              {pendingResume && <span style={{ fontSize: "0.8rem", color: "#ff9800" }}>⚠ Not saved yet — click Save Changes</span>}
            </div>
            <small style={{ color: "var(--muted)" }}>PDF only · Max 10 MB · Will be auto-attached when applying to jobs</small>
          </div>
      ...
      <div style={{ marginTop: "50px", maxWidth: "600px" }}>
        ...
        <form onSubmit={handlePasswordSubmit}>
          ...
          <button type="submit" className="btn btn-primary" disabled={pwLoading} style={{ marginTop: "10px" }}>
